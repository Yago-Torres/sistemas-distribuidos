...
// Método RPC que será llamado por los nodos
func (nr *NodoRaft) Barrier(args *BarrierArgs, reply *BarrierReply) error {
	nr.barreraMutex.Lock()
	defer nr.barreraMutex.Unlock()

	nr.nodosEnBarrera++
	nr.Logger.Printf("Nodo %d llegó a la barrera. Total: %d/%d\n",
		args.NodeId, nr.nodosEnBarrera, len(nr.Nodos))

	if nr.nodosEnBarrera == len(nr.Nodos) {
		reply.Ok = true
		nr.nodosEnBarrera = 0 // Reset para futuras barreras
	} else {
		reply.Ok = false
	}
...
// Método que llama cada nodo para sincronizarse
func (nr *NodoRaft) esperarEnBarrera() {
	args := &BarrierArgs{NodeId: nr.Yo}
	reply := &BarrierReply{}

	for {
		// Llamar a todos los nodos
		for i := range nr.Nodos {
			err := nr.Nodos[i].CallTimeout("NodoRaft.Barrier", args, reply, 100*time.Millisecond)
			if err == nil && reply.Ok {
				nr.Logger.Printf("Nodo %d liberado de la barrera\n", nr.Yo)
				return
			}
		}
